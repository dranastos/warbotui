...
contract NanoNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable  {
    ...
    constructor() ERC721("MicroMachineWarBots", "MMWarBot") {
        ...
        warbotsmanufacturercontract = WarBotManufacturer ( warbotsmanufacturer );
    ...
    function  oracleWriter ( uint256 _cardnumber, uint8 _cardtype, uint256 _bonus , uint256 _duration ,  uint256 _minimumlevel, uint8 _range, string memory _rarity ) public onlyOracle {
         ...
         NFTNanocards[_cardnumber].duration = _duration;
    ...
    function deployNFTNanoset( uint256 _warbot ) public payable {
        ...
        for ( uint i = 0; i < cardsperwarbot; i++ ) {
            ...
            NFTNanocards[newTokenId].creator = _creator;
            NFTNanocards[newTokenId].warbotbornof = _warbot;
            setBidExpiration ( newTokenId, block.timestamp + auctionPeriod );
            
        }
    }
    
    function activateNanoNFT ( uint256 _nftcard ) public {
        require ( msg.sender == ownerOf ( _nftcard ));
        ERC20 _nanomachines = ERC20 ( nanomachines );
        _nanomachines.transferFrom ( msg.sender , address(this), activationcost );
        uint256 _split = activationcost.div(2);
        NFTNanocards[ _nftcard ].activated = true;
        _nanomachines.transfer ( NFTNanocards[ _nftcard ].creator, _split );
        _nanomachines.transfer ( burnAddress, _split );
    ...
    function setBidExpiration ( uint256 _tokenId, uint256 _bidExpiration  ) internal {
        bidExpiration[_tokenId] = _bidExpiration;
    }
    
    
    function auctionBid( uint256 _tokenId, uint256 _amount ) public {
        require  ( bidExpiration[_tokenId ] > block.timestamp );
        require ( winnerDeclared[_tokenId] == false );
        uint256 _check = checkBids ( _tokenId, msg.sender );
        ERC20 _nanomachines = ERC20 ( nanomachines );
        _nanomachines.transferFrom( msg.sender, address(this ), _amount );
        
        
        if ( _check > 0 ) {
           
             bids[_tokenId][bidCount[_check]]._amount += _amount;
        } else {
             bidCount[_tokenId]++;
             bids[_tokenId][bidCount[_tokenId]]._bidder  = msg.sender;
             bids[_tokenId][bidCount[_tokenId]]._amount = _amount;
        }
      
    }
    
    function checkBids (  uint256 _tokenId, address _bidder ) public view returns(uint256) {
         
          for ( uint i = 1; i <= bidCount[_tokenId]; i++ ) {
             if ( _bidder == bids[_tokenId][i]._bidder ) return i;
          }
        return 0;
    }
    ...
    function isAuctionExpired( uint256 _tokenId ) public view returns(bool) {
        ...
        if ( block.timestamp > bidExpiration[ _tokenId] ) return true;
        return false;
    }
    
    function DeclareWinner( uint256 _tokenId ) public {
        require ( winnerDeclared[_tokenId] == false );
        require ( block.timestamp > bidExpiration[_tokenId], "Auction Still Live" );
        winnerDeclared[_tokenId] = true;
        ...
        uint256 _amount = _highestbid;
        ERC20 token = ERC20 ( nanomachines );
        if( _amount >0 ) token.transfer ( burnAddress, _amount );
        
        
     }
   uint256 public STATS;
   function setStats ( uint256 _stats ) public {
       
       STATS = _stats;
       
   }
   
   function highestBidder ( uint256 _tokenId ) public view returns ( address, uint256, uint256 ){
        ...
        for ( uint i = 1; i <= bidCount[_tokenId]; i++ ) {
             if ( bids[_tokenId][i]._amount > _highestbid ) {
                 _highestbid = bids[_tokenId][i]._amount;
                 _winning =  bids[_tokenId][i]._bidder;
                 _bidnumber = i;
             }
        }
       
       return ( _winning , _highestbid, _bidnumber );
   }
   ...
   function biddersBids ( address _address, uint256 _tokenId ) public view returns (  uint256, uint256, bool  ){
       ...
       return  (_bidamount, _bidnumber, isAuctionExpired(_tokenId) );
   }
   
    function WithdrawBid( uint256 _tokenId ) public {
        require ( winnerDeclared[_tokenId] == true );
        for ( uint i = 0; i < bidCount[_tokenId]; i++ ) {
             if ( bids[_tokenId][i]._bidder == msg.sender && bids[_tokenId][i]._withdrawn == false  ) {
