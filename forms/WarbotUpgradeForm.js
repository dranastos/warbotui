...
import Upgrade from '../components/warbots/Upgrade/Upgrade';
...
const WarbotUpgradeForm = ({onComplete, address}) => {
	...
	return (
		...
		// 				<Form.Item name="warbot1" label="Warbot 1 ( Warbot being Upgraded - Stats transferred )"
		// 				           rules={[{required: true, message: 'Enter Warbot 1 Identification'}]}>
		//
		...
		// 				<Form.Item name="warbot2" label="Warbot 2 ( Warbot used for parts - Stats lost)"
		// 				           rules={[{required: true, message: 'Enter Warbot 2 Identification'}]}>
		...
		// 					<Button size="large"
		// 					        type={isapprovedforall == true && allowance >= nanorequirement ? sufficientlyApproved : insufficientlyApproved}
		// 					        onClick={isapprovedforall && allowance >= nanorequirement == true && parseInt(warbot1level) == parseInt(warbot2level) && parseInt(warbot1) != parseInt(warbot2) ? handleDeposit : notEnough}>Upgrade
		// 						Warbots</Button>
		// 				</Space>
		// 				<Card style={{marginTop: 20, textAlign: 'center'}}>
		//
		// 					<Text level={5} strong> Upgrade Warbot # {warbot1} Level {warbot1level} Warbot to a
		// 						level {parseInt(warbot1level) + 1} Warbot by trading in an additional level {warbot1level} WarBot along
		// 						with {nanorequirement} nanomachines</Text>
		// 				</Card>
		// 			</Form>
		// 		</Card>
		// 	</Card>
