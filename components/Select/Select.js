...
const Select = ({placeholder, options, setSelectData, selectData, setIsError, alternate = false}) => {
	...
	const optionsList = options.map((option, i) => {
		return (
			<div
				...
				{!alternate && <img src={option.img} alt=""/>}
				<p>{option.text}</p>
			</div>
		);
	});
	...
	return (
		<div
			ref={wrapperRef}
			className={!alternate ? styles.select : styles.select_alternate}
			onClick={() => setActiveMenu(!activeMenu)}
		>
			<div
				className={styles.select__placeholder}
			>
				{!alternate && <img src={options[index].img} alt=""/>}
			...
			<FontAwesomeIcon icon={!alternate ? faCaretDown : faAngleDown}/>
		</div>
	);
};
