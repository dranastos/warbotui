...
contract Nanosales  is Ownable{
   ...
   function purchase ( uint256 _amount) public payable {
       ...
       uint256 _devamount = (_busdamount.mul(5)).div(100); 
       devAmount = _devamount;
      
        _busd.transfer( devaddress, _devamount );
   } 
   
   function calculatePrice ( uint256 _amount ) public view returns (uint256, uint256) {
      uint256 _priceincrease =  _amount.mul( priceIncreasePerCoin ).div( 10**18 );
      uint256 newBaseprice = basePrice + _priceincrease;
      uint256 priceAtNewBasePrice = ( newBaseprice * _amount ).div( 10**18 );
      uint256 priceAtOldBasePrice = ( basePrice * _amount ).div( 10**18 );
      return ( ( priceAtNewBasePrice + priceAtOldBasePrice) /2, newBaseprice );
   }
   ...
   function withdrawNanos() public  onlyOwner {
       ...
       uint256 _amount = _token.balanceOf( address(this) ).sub(tokensSold);
       _token.transfer( msg.sender, _amount );
   }
