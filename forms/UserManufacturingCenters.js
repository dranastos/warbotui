...
const UserManufacturingCenters = ({ onComplete, address }) => {
  ...
  const renderDeposit = (id, key) => {
	...
	var mmstaked = "MicroMachines Staked:" + web3.utils.fromWei(vaults[id]._micromachinesstaked.toString(), 'nano')
            ...
            <Row style={{ marginTop: 10 }} gutter={[20, 20]}>
              <Col span={24}>
                <Space size="small" style={{ marginBottom: 10 }}>
                  ...
                  <Button
                    type="primary"
                    onClick={() => sendVaultTx('manufacture', vaults[id].address, wallet.account)}>
                    Manufacture WarBots
                  </Button>
                  <Button
                    type="danger"
                    onClick={() => sendVaultTx('unstakeMicroMachines', vaults[id].address, wallet.account)}>
                    Shutdown Plant
                  </Button>
                  </Space>
...

              {
                Object.keys(vaults[id]).map((name, key) => (
                  <Col
                    key={`${id}-${name}-${key}`}
                    span={vaults.[id]['_micromachinesstaked'].toString().startsWith('0x') ? 24 : 8}>
                    <Statistic
