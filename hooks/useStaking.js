...
const useStaking = () => {
    ...
    const { warbotmanufacturer, connected } = useMicroMachineManufacturingPlant(
    ...
    const getVault = async (rawdata) => {
        const timeperiod = await warbotmanufacturer
            .manufacturingPeriod()
            .call();
        ...
        return {
            MicroMachinesStaked: web3.utils.fromWei(
                micromachinesstaked.toString(),
                "nano"
            ),
            PlantStatus: status ? "Active" : "Inactive",
            "NextBatch/Shutdown":
                parseInt(lastmanufacture) + parseInt(timeperiod) > timeNow
                    ? "Not Ready"
                    : "Ready",
            "Last Manufacture":
                lastmanufacture == timeinitiated
                    ? "Manufacturing Initiated"
                    : moment.unix(lastmanufacture).format("MM/DD/YYYY HH:mm"),
            "Time At Deposit": moment
                .unix(timeinitiated)
                .format("MM/DD/YYYY HH:mm"),
            timeAtExpiration: moment
                .unix(timeofexpiration)
                .format("MM/DD/YYYY HH:mm"),
